<?php namespace SamPoyigi\Reservation\Components;

class SeatBooker extends \System\Classes\BaseComponent
{
    ...
    public function index()
    {
        if ($this->config->item('reservation_mode') != '1') {
            $this->alert->set('alert', lang('alert_reservation_disabled'));
            $this->redirect('home');
        }
        ...
        $data['guest_numbers'] = ['2', '3', '4', '5', '6', '7', '8', '9', '10'];
        ...
        $data['reservation_alert'] = $this->alert->display('sampoyigi.reservation');

        $this->load->view('sampoyigi.reservation/sampoyigi.reservation', $data);
    }
    ...
    protected function findTable()
    {
        if ($this->validateForm() === TRUE) {
            ...
            if ($response === 'NO_ARGUMENTS') {
                ...
                $this->alert->set('alert_now', lang('alert_no_table_available'), 'sampoyigi.reservation');
            }
            else if ($response === 'FULLY_BOOKED') {
                $this->alert->set('alert_now', lang('alert_fully_booked'), 'sampoyigi.reservation');
            }
    ...
    protected function validateForm()
    {
        ...
        $this->form_validation->set_rules('location', 'lang:label_location', 'required|integer');
        $this->form_validation->set_rules('guest_num', 'lang:label_guest_num', 'required|integer');
        $this->form_validation->set_rules('reserve_date', 'lang:label_date', 'required|date|callback__validate_date');
        $this->form_validation->set_rules('reserve_time', 'lang:label_time', 'required|valid_time|callback__validate_time');

        if ($this->input->get('action') === 'select_time') {
            $this->form_validation->set_rules('selected_time', 'lang:label_time', 'required|valid_time|callback__validate_time');
        }
    ...
    public function _validate_date($str)
    {
        if (strtotime($str) < time()) {
            $this->form_validation->set_message('_validate_date', lang('error_indate'));
    ...
    public function _validate_time($str)
    {
        ...
        if (!empty($str)) {
            ...
            $this->form_validation->set_message('_validate_time', lang('error_invalid_time'));
    ...
    {
        $this->addCss(extension_url('sampoyigi.reservation/assets/stylesheet.css'), 'reservation-module-css');
        $this->addCss(assets_url('js/datepicker/datepicker.css'), 'datepicker-css');
        $this->addJs(assets_url("js/datepicker/bootstrap-datepicker.js"), 'bootstrap-datepicker-js');
        $this->addCss(assets_url('js/datepicker/bootstrap-timepicker.css'), 'bootstrap-timepicker-css');
        $this->addJs(assets_url("js/datepicker/bootstrap-timepicker.js"), 'bootstrap-timepicker-js');
    }
}
