...
it('loads reservation with associated customer and location', function() {
...
it('returns reservation by hash for specific customer', function() {
    $customer = Mockery::mock(Customer::class)->makePartial();
...
it('returns empty collection if location is not set when making time slots', function() {
    expect((new BookingManager())->makeTimeSlots(now()))->toBeEmpty();
});

it('returns time slots with default interval and lead time', function() {
...
it('returns time slots with custom interval and lead time', function() {
    $this->travelTo(Carbon::now()->startOfDay());
    $location = Mockery::mock(Location::class)->makePartial();
    $schedule = Mockery::mock(WorkingSchedule::class)->makePartial();
...
it('filters out past time slots based on lead time', function() {
    $this->travelTo(Carbon::now()->startOfDay());
    $location = Mockery::mock(Location::class)->makePartial();
    $schedule = Mockery::mock(WorkingSchedule::class)->makePartial();
...
it('saves reservation with provided data', function() {
    Event::fake();
...
it('returns false if not fully booked on given date and time', function() {
    Event::fake();
...
it('returns next bookable table for given date and time and number of guests', function() {
    $location = Mockery::mock(Location::class)->makePartial();
