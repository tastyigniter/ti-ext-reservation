...
it('validates rules correctly', function(): void {
    ...
    expect($rules['is_enabled'])->toBe(['boolean'])
        ...
        ->and($rules['min_advance_time'])->toBe(['integer', 'min:0', 'max:999'])
        ->and($rules['max_advance_time'])->toBe(['integer', 'min:0', 'max:999'])
        ->and($rules['cancellation_timeout'])->toBe(['integer', 'min:0', 'max:999']);
});
